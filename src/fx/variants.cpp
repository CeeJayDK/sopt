...
std::string switchName(const Region& r) {
  ...
  return "SOPT_" + stem + "_" + std::to_string(r.line);
}
...
std::vector<fs::path> writeVariants(const std::vector<RegionResult>& results,
  ...
  for (auto& [file, regs] : byFile) {
    ...
    std::sort(regs.begin(), regs.end(),
              [](const RegionResult* a, const RegionResult* b) { return a->region.line < b->region.line; });
    std::string out =
        ...
        "// statement; SOPT_ALL = 1 selects the first alternative everywhere.\n"
        "#ifndef SOPT_ALL\n#define SOPT_ALL 0\n#endif\n";
    uint32_t next = 1;  // next source line to copy
    for (const RegionResult* rr : regs) {
      const Region& r = rr->region;
      if (r.line < next || r.lastLine > lines->size()) {
        errors += file + ":" + std::to_string(r.line) + ": overlapping region (skipped)\n";
        continue;
      }
      for (; next < r.line; ++next) out += (*lines)[next - 1] + "\n";
      const std::string sw = switchName(r);
      const std::string ind = indentOf((*lines)[r.line - 1]);
      out += "#ifndef " + sw + "\n#define " + sw + " SOPT_ALL // 0 = original, 1.." +
             std::to_string(rr->variants.size()) + " = variants\n#endif\n";
      ...
      out += "#else\n";
      for (; next <= r.lastLine; ++next) out += (*lines)[next - 1] + "\n";
      out += "#endif\n";
    }
...
std::string markdownReport(const std::vector<RegionResult>& results, const ReportInfo& info) {
  ...
  for (const auto& rr : results) {
    ...
    s += "### `" + switchName(r) + "` — " + pathFrom(r.file).filename().string() + ":" +
         std::to_string(r.line) + " (" + r.function + ")\n\n";
    s += "```hlsl\n" + r.original + "\n```\n\n";
    s += "Inputs:\n";
  ...
  s += "## Regions without cheaper variants\n\n";
  for (const auto& rr : results) {
    if (!rr.variants.empty()) continue;
    const Region& r = rr.region;
